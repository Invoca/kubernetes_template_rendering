...
require_relative "../../../lib/invoca/kubernetes_templates/template"
require_relative "../../../lib/invoca/kubernetes_templates/erb_template"
require_relative "../../../lib/invoca/kubernetes_templates/jsonnet_template"

RSpec.describe Invoca::KubernetesTemplates::Template do
  let(:path) { "/tmp/foo.erb" }
  let(:hash) { { "a" => "value1", "b" => "value2" } }
  let(:erb) { "<%= a %> <%= b %>" }
  let(:klass) { Invoca::KubernetesTemplates::ErbTemplate }

  describe "#render" do
    let(:return_erb_on_file_read) { expect(File).to receive(:read).with(path).and_return(erb) }
    let(:template) { klass.new(path, hash) }
    ...
    context "for .jsonnet files" do
      let(:klass) { Invoca::KubernetesTemplates::JsonnetTemplate }
